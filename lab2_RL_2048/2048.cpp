...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for(int i=0;i<iso_last;i++)
			value+=(*this)[indexof(isomorphic[i],b)];	//weight table 查表
		return value;
	}

	/**
	 * update the value of a given board, and return its updated value
	 * gradient descent, update V(St)
	 * => V(St) <- V(St) + alpha * td_error
	 * td_error = (reward + V(St+1)) - V(St)
	 * u is equal to td_error that has multiplied alpha
	 */
	virtual float update(const board& b, float u) {
		// TODO
		float value=0;
		for(int i=0;i<iso_last;i++){
			value+=((*this)[indexof(isomorphic[i],b)]+=u);	//update the weights of weight table
		}
		return value;
	}
...
protected:
	// weighted table 查表, 回傳當下pattern所對應的weigt table哪個index
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				// update value V(St) towards TD target yt = Rt+1 + V(St+1)
				// 𝑠′, 𝑟 ← COMPUTE AFTERSTATE(𝑠, 𝑎)
				// 𝑆′′ ← ALL POSSIBLE NEXT STATES(𝑠′)
				// return 𝑟 + Σ𝑠′′∈𝑆′′𝑃(𝑠, 𝑎, 𝑠′′)𝑉(𝑠′′)
				// P is transition function, 2 is 0.9, 4 is 0.1
				// action = argmax[a](R[t+1]+E[V(S[t+1])])
...
int main(int argc, const char* argv[]) {
	...
	// record score to draw learning curve
	std::ofstream score_record("score_record.txt");
	if(!score_record)
	{
	...
	for (size_t n = 1; n <= total; n++) {
		...
		avg_score+=score;
		if(n % 200 == 0){
			std::cout<<"Write score to file"<<std::endl;
			score_record << n << " " << int(avg_score/200) << std::endl;
			avg_score=0;
		}
	}
